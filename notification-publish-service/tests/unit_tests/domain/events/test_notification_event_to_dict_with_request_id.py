...
def test_notification_event_to_dict_with_request_id():
    ...
    assert result_data["event_type"] == "NotificationRequested"
    assert result_data["data"]["user_id"] == "karlla12345"
    assert result_data["data"]["message"] == "recebemos seu pedido"
    assert result_data["data"]["channel"] == "email"
